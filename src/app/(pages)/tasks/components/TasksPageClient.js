...
import { getTasks, updateExistingTask } from '@/services/task';
...
const TasksPageClient = memo(() => {
    console.log("TasksPageClient - Component rendering");
    ...
    const filterAndSortTasks = useCallback(() => {
        ...
        console.log("TasksPageClient - Filtering and sorting tasks");
        ...
        console.log("TasksPageClient - Setting filtered tasks:", results.length);
        return results;
    ...
    const loadTasks = useCallback(async (force = false) => {
        ...
        if (loadingRef.current) {
            console.log("TasksPageClient - Already loading tasks, skipping");
            return;
        }
        ...
        if (!force && now - lastLoadTimeRef.current < 2000) {
            console.log("TasksPageClient - Throttling task reload, too frequent");
            return;
        }
        ...
        try {
            ...
            setIsLoading(true);
            console.log("TasksPageClient - Loading tasks...");

            const { data } = await getTasks();
            console.log("TasksPageClient - Tasks loaded:", data.length);
    ...
    useEffect(() => {
        if (!mountedRef.current) {
            console.log("TasksPageClient - Initial mount, loading tasks");
    ...
    useEffect(() => {
        if (mountedRef.current && refreshFlag > 0) {
            console.log("TasksPageClient - Refresh flag changed, reloading tasks");
            loadTasks(true);
        }
...
TasksPageClient.displayName = 'TasksPageClient';
